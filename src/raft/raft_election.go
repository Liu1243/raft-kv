...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	// check log, only grant vote when the candidate have more up-to-date log
...
func (rf *Raft) startElection(term int) bool {
	...
	askVoteFromPeer := func(peer int, args *RequestVoteArgs) {
		...
		if rf.contextLostLocked(Candidate, rf.currentTerm) {
			LOG(rf.me, rf.currentTerm, DLog, "Lost context, abort RequestVoteReply in T%d", rf.currentTerm)
			return
		}
